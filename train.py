...
def distributed():
    ...
    # sampler option is mutually exclusive to shuffle
    loader_train = DataLoader(dataset_train, batch_size=args.batch_size, shuffle=(train_sampler is None), num_workers=num_workers,
            collate_fn=t_util.collate_fn, sampler=train_sampler)
    loader_test = DataLoader(dataset_test, batch_size=args.batch_size, shuffle=(test_sampler is None), num_workers=num_workers,
            collate_fn=t_util.collate_fn, sampler=test_sampler)
    loader_valid = DataLoader(dataset_valid, batch_size=args.batch_size, shuffle=(val_sampler is None), num_workers=num_workers,
    ...
    def save(ep):
        new_name = '{}_{}_{}.pt'.format(model.__class__.__name__, epoch, ep - 1)
        print('{}/{}'.format(model_path, new_name))
        torch.save({
            'model': model.state_dict(),
            'epoch': ep
        }, str('{}/{}'.format(model_path, new_name)))
...
def single_gpu():
    ...
    model = get_model(num_classes)
    t_util.init_distributed_mode(args)
